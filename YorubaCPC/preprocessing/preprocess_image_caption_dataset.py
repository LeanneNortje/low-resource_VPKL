...
from image_caption_dataset_preprocessing import ImageCaptionDataset, AudioDataset
import json
...
heading(f'Reading Librispeech data.')
librispeech_fn = image_base / Path(args['librispeech_data_train'])
librispeech_train = list(librispeech_fn.rglob('*.flac'))
print(f'{len(librispeech_train)} data points found')

eng_speakers = {}
for key in english:
    for entry in english[key]:
        librispeech_train.append(english_audio_fn / entry['wav'])
        eng_speakers[english_audio_fn / entry['wav']] = entry["speaker"]

def SaveDatapointsOneDataloader(dataloader, subset, datasets):
    ...
    for i, (image_fn, eng_audio_feat, hindi_audio_feat, eng_audio_fn, hindi_audio_fn, image_path, eng_speaker) in enumerate(tqdm(dataloader, leave=False)):
        ...
        english_save_fn = save_fn / str("ENGLISH_" + eng_speaker[0] + "-" + eng_audio_fn[0].split("/")[-1].split(".")[0])
        hindi_save_fn = save_fn / str("HINDI_" + hindi_audio_fn[0].split("/")[-1].split(".")[0])
        ...
        if (eng_audio_feat.numpy() == 0).all() or (hindi_audio_feat.numpy() == 0).all():
            ...
            if not (Path("..") / english_save_fn).absolute().parent.is_dir(): (Path("..") / english_save_fn).absolute().parent.mkdir(parents=True)
            if not english_save_fn.is_file():   
                numpy.savez_compressed(
                    (Path("..") / english_save_fn).absolute(), 
                    eng_audio_feat=eng_audio_feat.squeeze().numpy()
                    )
                data_paths.append(str(english_save_fn)) 

            if not (Path("..") / hindi_save_fn).absolute().parent.is_dir(): (Path("..") / hindi_save_fn).absolute().parent.mkdir(parents=True)
            if not hindi_save_fn.is_file():   
                numpy.savez_compressed(
                    (Path("..") / hindi_save_fn).absolute(),
                    hindi_audio_feat=hindi_audio_feat.squeeze().numpy()
                    )
                data_paths.append(str(hindi_save_fn))   
...
def SaveDatapointsTwoDataloaders(dataloader, other_dataloader, subset, datasets):
    ...
    for i, (image_fn, eng_audio_feat, hindi_audio_feat, eng_audio_fn, hindi_audio_fn, image_path, eng_speaker) in enumerate(tqdm(dataloader, leave=False)):
        ...
        if (eng_audio_feat.numpy() == 0).all() or (hindi_audio_feat.numpy() == 0).all():
            ...
            if not english_save_fn.is_file():   
                numpy.savez_compressed(
                    (Path("..") / english_save_fn).absolute(), 
                    audio_feat=eng_audio_feat.squeeze().numpy()
                    )
                data_paths.append(str(english_save_fn)) 
            ...
            if not hindi_save_fn.is_file():   
                numpy.savez_compressed(
                    (Path("..") / hindi_save_fn).absolute(),
                    audio_feat=hindi_audio_feat.squeeze().numpy()
                    )
                data_paths.append(str(hindi_save_fn))   

    for i, (audio_feat, audio_name) in enumerate(tqdm(other_dataloader, leave=False)):

        save_fn = args["out_dir"]/Path(datasets)
 
        save_fn = save_fn / str(audio_name[0])

        if (audio_feat.numpy() == 0).all():
            print(f'Audio is zero: {audio_name[0]}')
        else:
...
other_train_loader = torch.utils.data.DataLoader(
    AudioDataset(
        librispeech_train, args["audio_config"], eng_speakers),
    batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
train_json_fn = SaveDatapointsTwoDataloaders(train_loader, other_train_loader, "train", "places+librispeech")
...
val_loader = torch.utils.data.DataLoader(
    ImageCaptionDataset(
        ...
        args["audio_config"]),
    batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
val_json_fn = SaveDatapointsOneDataloader(val_loader, "val", "places+librispeech")
...
test_loader = torch.utils.data.DataLoader(
    ImageCaptionDataset(
        ...
        args["audio_config"]),
    batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
test_json_fn = SaveDatapointsOneDataloader(test_loader, "test", "places+librispeech")
