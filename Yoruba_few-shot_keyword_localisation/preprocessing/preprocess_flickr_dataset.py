...
inverted_key = np.load(Path('../data/label_key.npz'), allow_pickle=True)['key'].item()
for id in key:
    print(id, key[id])
        ...
        for wav_name in raw_data[id]:
            ...
            for wav in raw_data[id][wav_name]:
                ...
                for sub_id in wav_to_id[wav_name]:
                    word_fn = (wav.parent / f'{wav.stem}_word_{sub_id}').with_suffix('.wav')
                    word_wavs[sub_id] = word_fn
                    
                image = '_'.join(wav_name.split('_')[1:-1])
                point = {
                    "speaker": speaker,
                    "wav_name": wav_name,
                    "wav": Path('..') / Path(wav),
...
def SaveDatapointsWithMasks(dataloader, subset, datasets):
    ...
    json_fn = (Path("..") / args["out_dir"]).absolute() / Path(datasets.split('_')[0] + "_" + subset + ".json")      
...
train_json_fn = SaveDatapointsWithMasks(train_loader, "train", 'yoruba')
...
val_loader = torch.utils.data.DataLoader(
    ...
    batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
val_json_fn = SaveDatapointsWithMasks(val_loader, "val", 'yoruba_val')