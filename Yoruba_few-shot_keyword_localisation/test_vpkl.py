...
def spawn_training(rank, world_size, image_base, args):
    ...
    if rank == 0: 
        ...
        # kayode = np.load(Path('data/yor-5k-init-preds/yor-5k-init/all_full_sigmoid_out.npz'), mmap_mode='r')
        # for wav_name in kayode.files:
        #     all_wav_names_kayode_used.append(speaker + '_' + wav_name)
        # print(len(all_wav_names_kayode_used))


        fn = Path(f'../Datasets/yfacc_v6/Flickr8k_text/Flickr8k.token.test_yoruba.txt')
        ...
        with open(fn, 'r') as f:
            for line in f:
                ...
                wav_fn = Path(args["path"]) / Path('yfacc_v6/flickr_audio_yoruba_test') / Path(str(wav_name) + '.wav')
        ...
        # print(len(set(all_wav_names_kayode_used)), len(set(list(yoruba_samples.keys()))))
        # print(set(all_wav_names_kayode_used) - set(list(yoruba_samples.keys())))
        # print(set(list(yoruba_samples.keys())) - set(all_wav_names_kayode_used))
        ...
        for txt_grid in Path('../Datasets/yfacc_v6/Flickr8k_alignment').rglob('*.TextGrid'):
            ...
            wav_fn = Path(args["path"]) / Path('yfacc_v6/flickr_audio_yoruba_test') / Path(str(wav) + '.wav')
            if wav_fn.is_file() is False: continue
            ...
            for interval in grid['words']:
                ...
                if label in yoruba_vocab:
                    ...
                    # if label in yoruba_vocab:
                    if wav not in yoruba_alignments: yoruba_alignments[wav] = {}
                    if label not in yoruba_alignments[wav]: yoruba_alignments[wav][label] = (float(start), float(end))
        print(len(yoruba_samples), len(yoruba_alignments))

        # print(len(set(all_wav_names_kayode_used)), len(set(list(yoruba_alignments.keys()))))
        # print(set(all_wav_names_kayode_used) - set(list(yoruba_alignments.keys())))
        # print(set(list(yoruba_alignments.keys())) - set(all_wav_names_kayode_used))
        ...
        with torch.no_grad():
            ...
            i = 1
            for wav_name in tqdm(yoruba_alignments):
                
                gt_trn = list(yoruba_alignments[wav_name].keys())
                # print(gt_trn)

                wav_fn = Path(args["path"]) / Path('yfacc_v6/flickr_audio_yoruba_test') / Path(str(wav_name) + '.wav') 

                target_dur = []
                for tok in yoruba_alignments[wav_name]:
                    start, end = yoruba_alignments[wav_name][tok]
                    target_dur.append((((float(start)*100)//2, (float(end)*100)//2), tok))
                ...
                sim, scores = attention.encode(image_output, None, english_output, english_nframes)
                # print(image_output.size(), english_output.size(), sim.size())
                # print(sim.size(), scores.size(), english_nframes.size())
                # scores = scores[:, :, 0:english_nframes[0]]

                # new_sim = torch.nn.functional.normalize(sim.unsqueeze(0)).squeeze(0)
                ...
                hyp_trn = list(set([iVOCAB[j] for j in np.where(similarities.cpu() >= threshold)[0]]))
                if i == 1:
                    print(similarities)
                    print(gt_trn)
                    print(hyp_trn)
                ...
                for word in hyp_trn:
                    ...
                    for n in range(scores.size(0)): 
                        word_score = scores.cpu().numpy()[n, 0:english_nframes[n]]
