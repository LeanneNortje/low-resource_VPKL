...
class ImageAudioDatawithMasks(Dataset):
    ...
    def _SamplePosImage(self, wav_name, im_path, id):
        
        num_p = 4
        a = self.pos_masks_to_images[id].copy()
        b = list(a.keys())
        if wav_name in b:  b.remove(wav_name)
        chosen_names = np.random.choice(b, size=3)
        temp = [i for name in chosen_names for i in a[name]]
        pos_img_fn = np.random.choice(temp, size=num_p)
        # print(pos_img_fn)
        positives = []

        for p in range(num_p):
            pos_wav_name= Path(pos_img_fn[p]).stem
            if wav_name == pos_wav_name or im_path == pos_img_fn[p]: 
                print("Positive sampling went wrong.")
                print(wav_name, pos_wav_name, '\n')

            fn = (Path('data/yoruba') / pos_wav_name).with_suffix('.npz')
            data_point = np.load(fn, allow_pickle=True)
            ...
            word_fn = Path(data_point['word_fns'].item()[id])
            word_feat = np.load(word_fn)["audio_feat"]
            word_feat, word_nframes = self._PadWordFeat(word_feat)

            positives.append(
                {"pos_audio": english_audio_feat1, "pos_frames": english_nframes1, "pos_image": img1, 
                "pos_word": word_feat, "pos_word_nframes": word_nframes
                })
            # break

        return positives

    def _SampleNegImage(self, wav_name, im_path, id):
        num_n = 4
        negatives = []
        all_ids = list(self.neg_masks_to_images[id].keys())
        neg_id = np.random.choice(all_ids, size=num_n)

        for n in range(num_n):
            temp = self.neg_masks_to_images[id][neg_id[n]].copy()

            neg_img_fn = np.random.choice(temp, size=1)[0]
            neg_wav_name = Path(neg_img_fn).stem
            if wav_name == neg_wav_name or im_path == neg_img_fn or neg_id[n] == id : 
                print("Negative sampling went wrong.")
                print(wav_name, neg_wav_name, '\n')

            fn = (Path('data/yoruba') / neg_wav_name).with_suffix('.npz')
            data_point = np.load(fn, allow_pickle=True)
            english_audio_feat1 = data_point["audio_feat"]
            english_audio_feat1, english_nframes1 = self._PadFeat(english_audio_feat1)
            imgpath = self.image_base_path / str(data_point['image'])
            ...
            # print(data_point['word_fns'].item().keys())
            # neg_id_temp = np.random.choice(list(data_point['word_fns'].item().keys()), size=1)[0]
            word_fn = Path(data_point['word_fns'].item()[neg_id[n]])
            word_feat = np.load(word_fn)["audio_feat"]
            word_feat, word_nframes = self._PadWordFeat(word_feat)

            negatives.append({
                "neg_audio": english_audio_feat1, "neg_frames": english_nframes1, "neg_image": img1, "neg_id": neg_id[n], 
                "neg_word": word_feat, "neg_word_nframes": word_nframes
                })
    ...
    def __getitem__(self, index):
        ...
        data_point = np.load(self.data[index], allow_pickle=True)
        wav_name = Path(self.data[index]).stem
        ...
        p_dict = self._SamplePosImage(wav_name, self.data[index], id)
        ...
        n_dict = self._SampleNegImage(wav_name, self.data[index], id)
        
        return {"image": image, "english_feat": english_audio_feat, "english_nframes": english_nframes, 
        "word_feat": word_feat, "word_nframes": word_nframes,
        "positives": p_dict, #[{"pos_audio": pos_english_audio_feat1, "pos_frames": pos_english_nframes1, "pos_image": pos_image1}],
        "negatives": n_dict, #[{"neg_audio": neg_english_audio_feat1, "neg_frames": neg_english_nframes1, "neg_image": neg_image1, "neg_id": neg_id1}, 
...
class ImageAudioDatawithMasksVal(Dataset):
    ...
    def _SamplePosImage(self, wav_name, im_path, id):
        ...
        for p in range(num_p):
            ...
            fn = (Path('data/yoruba_val') / pos_wav_name).with_suffix('.npz')
            data_point = np.load(fn, allow_pickle=True)
            english_audio_feat1 = data_point["audio_feat"]
            english_audio_feat1, english_nframes1 = self._PadFeat(english_audio_feat1)
            imgpath = self.image_base_path / str(data_point['image'])
            ...
            # word_fn = Path(data_point['word_fns'].item()[id])
            # word_feat = np.load(word_fn)["audio_feat"]
            # word_feat, word_nframes = self._PadFeat(word_feat)
    ...
    def _SampleNegImage(self, wav_name, im_path, id):
        ...
        for n in range(num_n):
            ...
            fn = (Path('data/yoruba_val') / neg_wav_name).with_suffix('.npz')
            data_point = np.load(fn, allow_pickle=True)
            english_audio_feat1 = data_point["audio_feat"]
            english_audio_feat1, english_nframes1 = self._PadFeat(english_audio_feat1)
            imgpath = self.image_base_path / str(data_point['image'])
            ...
            negatives.append({"neg_audio": english_audio_feat1, "neg_frames": english_nframes1, "neg_image": img1, "neg_id": neg_id[n], 
                # "neg_word": word_feat, "neg_word_nframes": word_nframes
                })
