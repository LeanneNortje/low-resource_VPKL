...
class flickrData(Dataset):
    ...
    def __getitem__(self, index):
        datum = self.data[index]
        ids = datum['ids']
        wav_fn = Path(datum['wav'])
        imgpath = (self.image_fn / Path(datum['image'])).with_suffix('.jpg')
        ...
        word_feats = {}
        for id in datum['word_wavs']:
            word_feat = self._LoadAudio(Path('..') / datum['word_wavs'][id])
            word_feats[id] = {'feat': word_feat, 'fn': str(datum['word_wavs'][id])}

        return imgpath, audio_feat, wav_name, datum['image'], datum["speaker"], ids, word_feats
