...
def spawn_training(rank, world_size, image_base, args):
    ...
    flickr_boundaries_fn = base.parent / Path('flickr_audio/flickr_8k.ctm')
    flickr_audio_dir = flickr_boundaries_fn.parent / "wavs"
    flickr_images_fn = Path('../Datasets') / Path('Flicker8k_Dataset')
    ...
    if rank == 0: 
        ...
        image_model = image_model_name(args, pretrained=False).to(rank)
        ...
        if "restore_epoch" != -1:
            info, start_epoch, global_step, best_epoch, best_acc = loadModelAttriburesAndTrainingAtEpochAMP(
                args["exp_dir"], audio_model, word_encoder, image_model, attention, contrastive_loss, optimizer, rank, args["restore_epoch"]
                )
            if rank == 0: print(f'\nEpoch particulars:\n\t\tepoch = {start_epoch}\n\t\tglobal_step = {global_step}\n\t\tbest_epoch = {best_epoch}\n\t\tbest_acc = {best_acc}\n')
        else:
            info, start_epoch, global_step, best_epoch, best_acc = loadModelAttriburesAndTrainingAMP(
                args["exp_dir"], audio_model, word_encoder, image_model, attention, contrastive_loss, optimizer, rank
                )
            if rank == 0: print(f'\nEpoch particulars:\n\t\tepoch = {start_epoch}\n\t\tglobal_step = {global_step}\n\t\tbest_epoch = {best_epoch}\n\t\tbest_acc = {best_acc}\n')

        
        image_base = Path('../Datasets') / Path('Flicker8k_Dataset/')
        ...
        with torch.no_grad():
            ...
            for word in images_for_keywords:
                for im_fn, _ in images_for_keywords[word]:
                    image = LoadImage(
                        Path('../Datasets') / Path('Flicker8k_Dataset') / Path((im_fn.stem) + '.jpg'), resize, image_normalize, to_tensor)
            ...
            for entry in tqdm(samples):
                ...
                wav_fn = base.parent / Path('flickr_audio/wavs') / Path(str(Path(entry['wave']).stem) + '.wav') 
                ...
                scores, sim = attention.encode(image_output, english_output, english_nframes)
                print(image_output.size(), english_output.size(), scores.size(), sim.size())
