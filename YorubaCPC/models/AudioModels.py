...
class Encoder(nn.Module):
    def __init__(self, channels=512):
        ...
        self.conv2 = nn.Conv1d(channels, channels, 1, stride=1, padding=0)
        self.norm2 = ChannelNorm(channels)
        self.conv3 = nn.Conv1d(channels, channels, 1, stride=1, padding=0)
        self.norm3 = ChannelNorm(channels)
        self.conv4 = nn.Conv1d(channels, channels, 1, stride=1, padding=0)
...
class Context(nn.Module):
    def __init__(self, z_dim, c_dim):
        super().__init__()
        self.lstm1 = nn.LSTM(z_dim, c_dim, batch_first=True)
        self.lstm2 = nn.LSTM(c_dim, c_dim, batch_first=True)
        self.lstm3 = nn.LSTM(c_dim, c_dim, batch_first=True)
        self.lstm4 = nn.LSTM(c_dim, c_dim, batch_first=True)
        self.h = None
...
class CPCEncoder(nn.Module):
    def __init__(self, z_dim, c_dim):
        ...
        self.context = Context(self.z_dim, self.c_dim)
