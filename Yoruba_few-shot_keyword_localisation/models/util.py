...
def loadPretrainedMultimodalWeights(audio_model, image_model, args, rank):

	checkpoint_fn = Path(f'pretrained/best_ckpt.pt')
	checkpoint = torch.load(checkpoint_fn, map_location={'cuda:%d' % 0: 'cuda:%d' % rank})

	# audio_model = nn.DataParallel(audio_model)
	# model_dict = audio_model.state_dict()

	# for key in checkpoint["audio_model"]:
	# 	if key in model_dict: model_dict[key] = checkpoint["audio_model"][key]
	# audio_model.load_state_dict(model_dict)

	image_model = nn.DataParallel(image_model)
	model_dict = image_model.state_dict()

	for key in checkpoint["image_model"]:
		if key in model_dict: model_dict[key] = checkpoint["image_model"][key]
	image_model.load_state_dict(model_dict)

	return audio_model, image_model.module
